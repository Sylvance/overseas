...
$requestSchemaFile=" Add VoidShipmentRequest Schema File";
$responseSchemaFile=" Add VoidShipmentResponse Schema File";
...
#print $schema->template('PERL' => 'VoidShipmentRequest');
$doc = XML::LibXML::Document->new('1.0', 'UTF-8');
$writer = $schema->compile(WRITER => 'VoidShipmentRequest');

my $voidshipmentrequest =  
{
	Request =>
	{
		RequestAction => '1',
...
$xml = $writer->($doc , $voidshipmentrequest);
$doc->setDocumentElement($xml);
push(@XML , $doc->toString());
